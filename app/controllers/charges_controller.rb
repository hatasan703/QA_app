class ChargesController < ApplicationController
    ...
    def identification
      ...
      def create_identification
        ...
        current_user.update(connect_id: account[:id])
        @user = User.find(params[:id])
        redirect_to controller: 'charges', action: 'identification', id: @user.id
      end
      ...
      def create_bank
        ...
        redirect_to controller: 'charges', action: 'bank', id: @user.id
      end
      ...
      def payout_confirmation
        @user = User.find(params[:id])
        if @user.money < 50
            redirect_to controller: 'charges', action: 'point', id: @user.id
      ...
      def payout_point
        ...
        if current_user.connect_id.present?
            account = Stripe::Account.retrieve(current_user.connect_id)
            if account.external_accounts.data.present? && @user.money >= 50
